...
public class UpdateHandler(ITelegramBotClient bot, ILogger<UpdateHandler> logger) : IUpdateHandler
{
    ...
    private async Task<Message> CheckNeedSlowdownOrSpeedup(Message msg)
    {
        ...
        if (msg.Type == MessageType.Text)
        {
            ...
            {
                if (message.ContainsAny("притихни", "затихни", "полегче", "остынь", "успокойся", "притормози", "потише", "сбавь обороты", "тормози", "не горячись", "помедленнее", "спокойнее", "поаккуратнее", "притормозь", "не спеши", "попридержи коней", "остановись", "поуймись", "умерь пыл", "не торопись", "поосторожнее", "притихни-ка", "притуши", "хорош"))
                {
                ...
                else if (message.ContainsAny("заткнись", "умолкни", "тихо", "замолчи", "прекрати", "стоп", "молчи", "перестань", "стопэ", "утихни", "шшш", "хватит", "закрой рот", "не говори", "молчать"))
                {
                    if (MasterRandomScore > 0)
                    {
                        MasterRandomScore = 0;
            ...
            if (message.ContainsAny("бот живи", "бот говори") && MasterRandomScore == 0)
            {
                MasterRandomScore = 1;
                speedChanged = true;
            }
